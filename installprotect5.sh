...
BRAND_NAME="${BRAND_NAME:-MasCarli Official}"
BRAND_TEXT="${BRAND_TEXT:-Protect By MASCARLI}"
CONTACT_TELEGRAM="${CONTACT_TELEGRAM:-@MasCarli}"
BOT_LINK="${BOT_LINK:-@MasCarlStorebot}"
WELCOME_TITLE="${WELCOME_TITLE:-Welcome To Server $BRAND_NAME}"
WELCOME_MESSAGE="${WELCOME_MESSAGE:-Butuh panel yang anti mokad? langsung aja ke $BOT_LINK. Jika ada kendala dan ada yang ingin di tanyakan hubungi $CONTACT_TELEGRAM.}"
...
cleanup_old_branding() {
  ...
  remove_block_by_markers "$file" "<!-- BRANDING_MASCARLI_START -->" "<!-- BRANDING_MASCARLI_END -->"
  remove_block_by_markers "$file" "<!-- BRANDING_MASCARLI: Custom Branding -->" "</style>"

  tmp_file=$(mktemp)
  awk '
    BEGIN { skip=0; depth=0; seen_div=0 }
    /<!-- BRANDING_MASCARLI: Footer -->/ { skip=1; depth=0; seen_div=0; next }
    skip {
      line=$0
      opens=gsub(/<div[^>]*>/, "&", line)
...
if "PROTEKSI_MASCARLI" in content:
...
while i < len(lines):
    ...
    if re.search(r'public function (?!__construct)', line):
        ...
        new_lines.append("        // PROTEKSI_MASCARLI: Hanya admin ID 1")
        new_lines.append("        if (!Auth::user() || (int) Auth::user()->id !== 1) {")
        new_lines.append("            abort(403, 'Akses ditolak - protect by MasCarli Tech');")
        new_lines.append("        }")
...
grep -n "PROTEKSI_MASCARLI" "$CONTROLLER"
echo ""
...
if [ -f "$EGG_CONTROLLER" ]; then
  if ! grep -q "PROTEKSI_MASCARLI" "$EGG_CONTROLLER"; then
...
while i < len(lines):
    ...
    if re.search(r'public function (?!__construct)', line):
        ...
        new_lines.append("        // PROTEKSI_MASCARLI: Hanya admin ID 1")
        new_lines.append("        if (!Auth::user() || (int) Auth::user()->id !== 1) {")
        new_lines.append("            abort(403, 'Akses ditolak - protect by MasCarli Official');")
        new_lines.append("        }")
...
for CLEANUP_FILE in "/var/www/pterodactyl/resources/views/layouts/master.blade.php" "/var/www/pterodactyl/resources/views/layouts/auth.blade.php"; do
  if [ -f "$CLEANUP_FILE" ] && grep -q "BRANDING_MASCARLI" "$CLEANUP_FILE" 2>/dev/null; then
...
for LF in "${LAYOUT_FILES[@]}"; do
  if [ -f "$LF" ]; then
    ...
    if grep -q "BRANDING_MASCARLI" "$LF" 2>/dev/null; then
      BRANDING_APPLIED=1
    fi
  fi
done
...
if [ -z "$WELCOME_TARGET" ] || [ ! -f "$WELCOME_TARGET" ]; then
  ...
  remove_block_by_markers "$WELCOME_TARGET" "<!-- WELCOME_MASCARLI: Welcome Banner -->" "<!-- /WELCOME_MASCARLI -->"

  WELCOME_TEMP=$(mktemp)
  cat > "$WELCOME_TEMP" << WELCOME_EOF
<!-- WELCOME_MASCARLI: Welcome Banner -->
<style>
  .jhonaley-welcome {
    background: linear-gradient(135deg, #0c1929, #132f4c, #0a2744);
...
if [ -f "$ADMIN_LAYOUT" ] && ! grep -q "PROTEKSI_MASCARLI_MASTER_SIDEBAR" "$ADMIN_LAYOUT" 2>/dev/null; then
  echo "🔧 Re-inject sidebar Protect Manager..."

  SIDEBAR_SNIPPET=$(mktemp)
  cat > "$SIDEBAR_SNIPPET" << 'SIDEBAR_PM_EOF'
                {{-- PROTEKSI_MASCARLI_MASTER_SIDEBAR: Protect Manager Menu --}}
                ...
                @endif
                {{-- END PROTEKSI_MASCARLI_MASTER_SIDEBAR --}}
SIDEBAR_PM_EOF
