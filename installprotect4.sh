...
class NodeController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, '🚫 Akses ditolak! Hanya admin ID 1 yang dapat membuka menu Nodes. ©Protect By MasCarli V1.0.0');
        }
        // ======================================================
...
sed -i "s|Protect By MasCarli|${BRAND_TEXT}|g" "$REMOTE_PATH" 2>/dev/null || true
sed -i "s|MasCarli Tech|${BRAND_NAME}|g" "$REMOTE_PATH" 2>/dev/null || true
