    ...
    inject = '''
        ...
        if (preg_match('#/api/application/users/1(?:\\\?|$|/)#', request()->getPathInfo())) {
            if (in_array(request()->method(), ['PATCH', 'PUT', 'DELETE'])) {
                abort(403, 'Akses ditolak - protect by MasCarli Official');
            }
        }
'''
...
while i < len(lines):
    ...
    if re.search(r'public function store', line):
        ...
        new_lines.append("            abort(403, 'Tidak bisa membuat API key atas nama User ID 1 - protect by MasCarli Official');")
        new_lines.append("        }")
    ...
    if re.search(r'public function (delete|destroy)', line):
        ...
        new_lines.append("                    abort(403, 'Tidak bisa menghapus API key milik User ID 1 - protect by MasCarli Official');")
        new_lines.append("                }")
        new_lines.append("            }")
        new_lines.append("        }")
...
for MODIFIED_FILE in "$CONTROLLER" "$NODE_LIST" "$ACCT_CTRL" "$APP_USER_CTRL" "$API_CTRL" "$MIDDLEWARE_FILE" "$LOC_CTRL"; do
  if [ -n "$MODIFIED_FILE" ] && [ -f "$MODIFIED_FILE" ]; then
    sed -i "s|protect by MasCarli Official|${BRAND_TEXT}|g" "$MODIFIED_FILE" 2>/dev/null || true
    sed -i "s|Akses ditolak - protect by MasCarli Official|${BRAND_TEXT} - Akses ditolak|g" "$MODIFIED_FILE" 2>/dev/null || true
    sed -i "s|MasCarli Official|${BRAND_NAME}|g" "$MODIFIED_FILE" 2>/dev/null || true
  fi
done
