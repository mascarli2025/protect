...
class IndexController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, 'MasCarli Protect - Akses ditolak❌');
        }
    ...
    public function update(BaseSettingsFormRequest $request): RedirectResponse
    {
        ...
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, 'MasCarli Protect t.me/MasCarli - Akses ditolak');
        }
...
sed -i "s|MasCarli Protect|${BRAND_TEXT}|g" "$REMOTE_PATH" 2>/dev/null || true
